...
public:
    ...
    void print(ofstream& outputFile) const {
        ...
        outputFile << "Cover URL: " << coverUrl << endl;
        outputFile << "Web Video URL: " << webVideoUrl << endl;
        outputFile << "Music Name: " << musicName << endl;
        outputFile << "Music Artist: " << musicArtist << endl;
        outputFile << "Music ID: " << musicID << endl;
        outputFile << "Play Count: " << playCount << endl;
        outputFile << "Post Text:" << endl;
        for (size_t i = 0; i < postText.size(); ++i) {
            outputFile << "- " << postText[i];
        }
        outputFile << endl;
        outputFile << endl;
    }
...
map<string, TikTokVideo> parseJSON(const string& filename) {
    ...
    string line;
    while (getline(file, line)) { // function to parse and take all key features needed for the program
...
map<string, pair<int64_t, int64_t>> countStringOccurrencesWithViews(const map<string, TikTokVideo>& videos) {
    map<string, pair<int64_t, int64_t>> occurrenceAndViewMap;
    ...
    for (map<string, TikTokVideo>::const_iterator it = videos.begin(); it != videos.end(); ++it) {
        ...
        for (vector<string>::const_iterator str_it = video.postText.begin(); str_it != video.postText.end(); ++str_it) {
            // Increment occurrences and add views
            occurrenceAndViewMap[*str_it].first++;  // Increment occurrences
            int64_t playCountSample = video.playCount;
            int64_t currentCount = occurrenceAndViewMap[*str_it].second;
            int64_t result = playCountSample + currentCount;
            occurrenceAndViewMap[*str_it].second = result; // Add views
        }
    }
...
void hashtagFunction(const map<string, pair<int64_t, int64_t>>& occurrenceAndViewMap, const map<string, TikTokVideo>& videos, ofstream& outputFile) {
    // custom comparison function to compare pairs of strings by occurrences, views, and string itself
    struct CustomCompare {
        bool operator()(const pair<string, pair<int64_t, int64_t>>& a, const pair<string, pair<int64_t, int64_t>>& b) const {
    ...
    // priority queue using custom comparison function
    priority_queue<pair<string, pair<int64_t, int64_t>>, vector<pair<string, pair<int64_t, int64_t>>>, CustomCompare> pq;

    // Push map elements into the priority queue
    for (map<string, pair<int64_t, int64_t>>::const_iterator it = occurrenceAndViewMap.begin(); it != occurrenceAndViewMap.end(); ++it) {
        pq.push(*it);
    }
    ...
    while (!pq.empty() && count < 10) {
        const pair<string, pair<int64_t, int64_t>>& pair = pq.top();
        outputFile << "========================" << endl;
        outputFile << "#" << pair.first << endl;
        outputFile << "used " << pair.second.first << " times" << endl;
        int64_t extract = pair.second.second;
        outputFile << to_string(extract) << " views" << endl;
        outputFile << endl;
        ...
        for (map<string, TikTokVideo>::const_iterator it = videos.begin(); it != videos.end(); ++it) {
            ...
            for (size_t i = 0; i < (video.postText).size(); ++i) {
                if((video.postText)[i] == pair.first){
                    holderMap[video.playCount + adding] = video;
                    adding += 1;
                }
            }       
        }
    ...
    for (map<string, TikTokVideo>::const_iterator it = videos.begin(); it != videos.end(); ++it) {
        ...
        if (musicName == "original sound") {
            ...
            if (musicOccurrences.find(make_pair("original sound", musicArtist)) != musicOccurrences.end()) {
                ...
                musicName = "original sound//x//" + to_string(number);
            } else {
        ...
        if (musicName == "sonido original") {
            ...
            if (musicOccurrences.find(make_pair("sonido original", musicArtist)) != musicOccurrences.end()) {
                ...
                musicName = "sonido original//x//" + to_string(number);
            } else {
...
void printTop10Musics(const map<string, TikTokVideo>& videos, ofstream& outputFile) {
    ...
    outputFile << "trending sounds:" << endl;
    outputFile << endl;
    int count = 0;
    for (vector<pair<pair<string, string>, pair<pair<int, int>, string>>>::const_iterator it = sortedMusics.begin(); it != sortedMusics.end(); ++it) {
        string soundName = it->first.first;
        // Check if the sound name ends with a space followed by a number
        if (!soundName.empty() && isdigit(soundName.back())) {
            // Find the last space in the string
            size_t pos = soundName.find_last_of("//x//");
            if (pos != string::npos) {
                // Extract the substring before the last space and the number suffix
                soundName = soundName.substr(0, pos-4), true;
            }
        }

        string soundArtist = it->first.second;
        outputFile << "========================" << endl;
        outputFile << soundName << endl;
        outputFile << it->second.first.second << " views" << endl;
        outputFile << soundArtist << endl;
        string soundID = it->second.second;
        outputFile << "music id: " << soundID << endl;
        outputFile << endl;
        ...
        // Reverse iterator to print the map in descending order
        int videoCount = 0;
        map<int, TikTokVideo>::reverse_iterator ite;
        for (ite = holderMap.rbegin(); ite != holderMap.rend(); ++ite) {
            outputFile << "cover url: " << ite->second.coverUrl << endl;
            outputFile << "web video url: " << ite->second.webVideoUrl << endl;
            ++videoCount;
            if (videoCount >= 3) break;
        }
        outputFile << "========================" << endl;
...
int main(int argc, char* argv[]) {
    ...
    if(command == "hashtag"){
        map<string, pair<int64_t, int64_t>> occurrenceAndViewMap = countStringOccurrencesWithViews(videos);
