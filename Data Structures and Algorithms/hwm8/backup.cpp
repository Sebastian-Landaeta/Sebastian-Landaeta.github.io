...
public:
    ...
    void print() const { //debugging class print
        ...
        std::cout << "Comment ID: " << comment_id << std::endl;
        std::cout << "Like Count: " << like_count << std::endl;
        std::cout << "Reply Count: " << reply_count << std::endl;
        std::cout << "Is Reply: " << (is_reply ? "true" : "false") << std::endl;
        std::cout << "Parent Comment ID: " << parent_comment_id << std::endl;
        std::cout << "Published Date: " << published_date << std::endl;
        std::cout << "Crawled Date: " << crawled_date << std::endl;
        std::cout << "Is Video Owner: " << (is_video_owner ? "true" : "false") << std::endl;
...
vector<Comment> delete_comment(std::vector<Comment> comments, const string& ID){
    ...
    for (std::vector<Comment>::iterator it = comments.begin(); it != comments.end(); ++it) {
        if (it->getParentCommentId() == ID) { // find any comments that had the comment as parent and unlink them
            cout << "deleted parent of comment --> " << it->getAuthor() << endl;
            it->removeParent();
        }
    }
    return comments;
...
void printComment(std::vector<Comment> comments, const string& ID, ofstream& output_file){
    ...
    comments[comments.size()-2].print();
